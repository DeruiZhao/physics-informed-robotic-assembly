...
TOOL_FRAMES = {"ur5e_robotiq": "gripper_center_link"}
DEFAULT_RESOLUTIONS = np.array([0.02, 0.02, 0.02, 0.05, 0.05, 0.05])
DEFAULT_WEIGHTS = np.reciprocal(DEFAULT_RESOLUTIONS)
...
##################################################
def get_arm_joints(robot, info):
    return joints_from_names(robot, info)
...
class BodyConf(object):
    ...
    def __init__(self, body, configuration=None, joints=None):
        if joints is None:
            joints = get_arm_joints(body, UR5E_JOINTS)
...
def get_ik_fn(robot, fixed=[], allowed_collisions=[], teleport=False, num_attempts=10):
    """
    ...
    movable_arm_joints = get_arm_joints(robot, UR5E_JOINTS)
    sample_fn = get_sample_fn(robot, movable_arm_joints)
    ...
    def fn(body, pose, grasp):
        ...
        for _ in range(num_attempts):
            set_joint_positions(robot, movable_arm_joints, sample_fn())  # Random seed
            # Attempt to find an IK solution for the approach pose
            q_approach = ur5e_robotiq_inverse_kinematics(
                robot,
                grasp.link,
                approach_pose,
                obstacles=obstacles,
            ...
            # Check for collisions, skipping allowed collisions
            if (q_approach is None) or any(
                pairwise_collision_with_allowed(
                    robot, b, allowed_collisions=allowed_collisions
                )
                for b in obstacles
            ):
                continue
            conf = BodyConf(robot, q_approach)
            # Attempt to find an IK solution for the grasp pose
            q_grasp = ur5e_robotiq_inverse_kinematics(
                robot,
                grasp.link,
                gripper_pose,
                obstacles=obstacles,
            ...
            if teleport:
                ...
                path = plan_direct_joint_motion(
                    ...
                    q_grasp,
                    obstacles=obstacles,
                    disabled_collisions=allowed_collisions,
                )
                if path is None:
                    if DEBUG_FAILURE:
