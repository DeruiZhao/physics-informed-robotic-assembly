...
# def approximate_as_prism_multi_link(body, body_pose=unit_pose(), **kwargs):
#     # TODO: make it just orientation
#     vertices = apply_affine(body_pose, vertices_from_body(body, **kwargs))
#     aabb = aabb_from_points(vertices)
#     return get_aabb_center(aabb), get_aabb_extent(aabb)
...
def plan_joint_motion(
    ...
    **kwargs
    ...
    # start_conf = get_joint_positions(body, joints)
    # Use provided start_conf if available, otherwise get current joint positions
    if start_conf is None:
        start_conf = get_joint_positions(body, joints)
    else:
        assert len(joints) == len(start_conf), "start_conf must match the number of joints"
