...
video_filename = os.path.join(output_dir, "column_assembly_planning_viz.mp4")
...
for seq_index, bar_id in enumerate(assembly_sequence):
    ...
    for i in range(10):
        ...
        if approach_paths is not None:
            ...
            print(f"Attempts for approach paths: {i + 1} - Path found with cost: {approach_cost}")
...
print(f"Stopped recording video to {video_filename}")